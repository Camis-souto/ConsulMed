...
namespace ConsulMed.Controllers
{
    ...
    public class BeneficiarioController : ControllerBase
    {
        ...
        public IActionResult Atualizar(BeneficiarioDto cadastrarDto)
        {
            try
            {
                int resultado = _beneficiarioRepositorio.Atualizar(cadastrarDto);

                if (cadastrarDto == null || cadastrarDto.IdBeneficiario < 1)
                    return NoContent();
        ...
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(int))]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public IActionResult AtualizarId(int id)
        {
            if(id == null || id < 1)
                return NoContent();
